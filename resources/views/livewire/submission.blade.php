    ...
    <div>
        ...
        <div class="card-body">
            <div class="w-70 mx-auto p-2 d-flex justify-content-around">
                <button type="button" class="rounded-circle border-1 bg-transparent align-center @if(Auth::user()->hasVotedPositively($item->id_sug)) voted @endif" value="1" style="height: 50px; width: 50px; border: 2px solid var(--secondary-color);">
                    <img src="{{Storage::url('img/Thumbs.svg')}}">
                </button>
                <button type="button" class="rounded-circle border-1 bg-transparent align-center @if(Auth::user()->hasVotedNegatively($item->id_sug)) voted @endif" value="0" style="height: 50px; width: 50px; border: 2px solid var(--secondary-color);">
